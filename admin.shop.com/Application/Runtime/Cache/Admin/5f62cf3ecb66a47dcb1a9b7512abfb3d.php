...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($menus)): foreach($menus as $key=>$top_menu): if(($top_menu["parent_id"]) == "0"): ?><li class="explode" key="" name="menu">
                    <?php echo ($top_menu["name"]); ?>
                        <ul>
                            <?php if(is_array($menus)): foreach($menus as $key=>$second_menu): if(($second_menu["parent_id"]) == $top_menu["id"]): ?><li class="menu-item"><a href="<?php echo U($second_menu['path']);?>" target="main-frame"><?php echo ($second_menu["name"]); ?></a></li><?php endif; endforeach; endif; ?>
                        </ul>
